RoutingError = function(message, code, opts) {
    return {
...
.get(function () {
    ...
    var data = Metadata.findOne(this.params.id);
    ...
    if (data) {
    	...
    	var error = RoutingError("Could not find metadata record with id " + this.params.id, 400);
		res.statusCode = 400;
		res.setHeader("Content-Type","application/json");
...
.get(function () {
	...
	var removedId = Metadata.remove(this.params.id);
